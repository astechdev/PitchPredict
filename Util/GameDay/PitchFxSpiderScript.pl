...
# script stopped at 12/01/08 so we will start again, get all important files from MLB.com, 8/22/07 through yesterday
$start = timelocal(0,0,0,27,2,109);
    ...
    foreach $game (@games) 
    {
        ...
        else 
        {
            ...
            if($gamehtml =~ m/<a href=\"game\.xml\"/ ) 
            {
                ...
                open GAME, ">$gamedir/game.xml" or die "could not open file $gamedir/game.xml: $|\n";
                print GAME $infohtml;
                close GAME;
            } 
            else
